function [dataSet_agg, dataset_pg_agg] = agg_spotwise_3Dgauss(gridFoVmm, dataSet, spotPGmm, sigma, direction)
%이 함수는 cyldinder 형태의 spot aggregation을 수행한다.

%%
if size(dataSet,1) ~= size(spotPGmm,1)
    error('data size not match (dataset_spot ~= dstaset pg');
end
% width_pg = size(dataset_pg,2);
...
for idx_query = 1:numSpot
    if dataSet_agg(idx_query, 14) ==0 % plan과 match되지 않은 스팟은 스킵한다.
        continue;
    end
    ...
    for idx_temp = 1:numSpot
        if dataSet_agg(idx_temp, 14) ==0 % plan과 match되지 않은 스팟은 스킵한다.
            continue;
        end
        ...
        dist_shift = interp1(xgrid_temp,spotPGmm(idx_temp,:),gridFoVmm);
    ...
    if sum(squeeze(dataset_pg_agg(idx_query, :))) == 0
        dataSet_agg(idx_query,15) = NaN;
    else
        dataSet_agg(idx_query,15) = GetRange_ver4p3(gridFoVmm, squeeze(dataset_pg_agg(idx_query, :)), 0, direction);
    end
end
